...
unsup_file_names, unsup_corpus, unsup_aggregate_corpus = \
    corpus_creator(input_directory_path = 'input_raw_data/twitter_chipotle_15/',\
        output_directory_path = 'output_parsed_data/chipotle_15_v4/')
...
# Number of Unique Words in unsup corpus 101768
...
while (length_test < 25):
    ...
    nkeys = nkeys + 1
print(nkeys)
# nkeys reached to 3057 to get 25 positive words; 
# nkeys is 487 to get 5 positive words
...
while (length_test < 25):
    ...
    nkeys = nkeys + 1
print(nkeys)
# nkeys is 1185 to get 25 negative words
# nkeys is 96 to get 25 negative words
...
# test subset of data
# positive, negative = compute_scores(unsup_corpus[0:2])
#
# text = unsup_corpus[0]
# good = score_positive(text)
# bad = score_negative(text)
# i = 0
# while (i < 92):
#    if (positive[i]> 5):
#        print('index:', i, positive[i], negative[i] )
#    i = i + 1
...
plt.savefig('fig_sentiment_text_measures_scatter_plot_chipotle15.pdf', 
...
plt.xlabel('days from 8/1/2015-10/31/2015(Chipotle)')
plt.ylabel('sentiment %, blue-positive, red-negative') 
plt.show()  
plt.savefig('fig_line_plot_chipotle15.pdf', 
    bbox_inches = 'tight', dpi=None, facecolor='none', edgecolor='blue', 
    orientation='portrait', papertype=None, format=None, 
    transparent=True, pad_inches=0.25, frameon=None) 
