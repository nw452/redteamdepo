...
def score_positive(text):    
    positive = [w for w in text.split() if w in positive_word_list]
#    positive = [w for w in text if w in positive_word_list]
    total = [w for w in text.split()]
#    total = [w for w in text]
    if len(total) > 0:
...
def score_negative(text):    
    negative = [w for w in text.split() if w in negative_word_list]
    total = [w for w in text.split()]
#    negative = [w for w in text if w in negative_word_list]
#    total = [w for w in text]   
    if len(total) > 0:
...
unsup_file_names, unsup_corpus, unsup_aggregate_corpus = \
    corpus_creator(input_directory_path = 'input_raw_data/twitter_taco_15/',\
        output_directory_path = 'output_parsed_data/taco_15_v3/')
...
while (length_test < 25):
    ...
    nkeys = nkeys + 1
print(nkeys)
# nkeys reached to 3081 to get 25 positive words; 
...
while (length_test < 25):
    ...
    nkeys = nkeys + 1
print(nkeys)
# nkeys is 1694 to get 25 negative words
...
plt.savefig('fig_sentiment_text_measures_scatter_plot_taco15.pdf', 
...
plt.xlabel('days from 8/1/2015 - 10/31/2015 (Taco Bell)')
plt.ylabel('sentiment %, blue-positive, red-negative') 
plt.show()  
plt.savefig('fig_line_plot_taco15.pdf', 
    bbox_inches = 'tight', dpi=None, facecolor='none', edgecolor='blue', 
    orientation='portrait', papertype=None, format=None, 
    transparent=True, pad_inches=0.25, frameon=None) 
